...
namespace {
	/* 图片设置 */
	const int Image_Width = 200;			// 图片宽度
	const int Image_Height = 200;			// 图片高度
...
void Cornell_smoke() {
	/* 相机 */
	{
		...
		double aperture = 0.0;
		double vfov = 40.0;
		main_camera = New<Camera>(from, at, Vec3(0, 1, 0), vfov, aspect, aperture, 0.7 * dist_to_focus, 0, 1);	// 主相机
	}
	...
	/* 场景 */
	{
		...
		world.Add(New<FlipNormal>(New<RectXZ>(213, 343, 227, 332, 554, light)));					// 顶部光源
